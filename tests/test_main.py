...
TOKENIZED_TEXT = [[101, 7592, 2088, 102], [101, 2023, 2003, 1037, 3231, 102], [101, 10924, 2638, 2003, 2307, 102]]
...
def test_load_data(mock_open):
    ...
    mock_open.return_value.__enter__.return_value.read.return_value = '[{"Line": "Hello world."}, {"Line": "ZenML is great!"}]'
    
    result = load_data()
    assert result == "Hello world. ZenML is great!", "load_data() output does not match expected text"
...
def test_encode_text():
    ...
    assert all(isinstance(seq, list) for seq in result), "Each element should be a list of token IDs"
    assert result == TOKENIZED_TEXT, "Tokenized output does not match expected BERT token IDs"
...
def test_split_data():
    ...
    assert "train" in result and "val" in result and "test" in result, "Missing keys in split data"
    assert len(result["train"]) > len(result["val"]) > len(result["test"]), "Incorrect data split ratios"
...
def test_train_model(mock_step, mock_zero_grad, mock_backward):
    ...
    assert "train_loss" in result, "train_model should return a dictionary with train_loss"
