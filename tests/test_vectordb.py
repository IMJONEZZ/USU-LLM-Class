...
def mock_embedder():
    embedder = MagicMock()
    embedder.encode.return_value = np.random.rand(384).astype(
        np.float32
...
def test_upsert_batch_content(mock_pinecone, mock_embedder):
    ...
    ingestion = WikiDataIngestion(mock_pinecone, embedder=mock_embedder, batch_limit=2)
...
def test_query_execution(mock_pinecone, mock_embedder):
    ...
    results = ingestion.index.query(vector=embeddings, top_k=3, include_metadata=True)

    # Verify call arguments
    mock_pinecone.query.assert_called_once_with(
        vector=embeddings, top_k=3, include_metadata=True
    )
