...
class TestChromaVectorDB:
    ...
    def test_initialization_persistent(self, temp_dir):
        ...
        collection_name = f"test_collection_{uuid.uuid4().hex[:8]}"
        db = ChromaVectorDB(collection_name=collection_name, persist_directory=temp_dir)
    ...
    def test_search(self):
        ...
        assert (
            "Artificial" in results["documents"][0]
            or "Machine" in results["documents"][0]
        )
    ...
    def test_search_with_filter(self):
        ...
        metadatas = [{"category": "AI"}, {"category": "Database"}, {"category": "AI"}]
        db.add_texts(texts, metadatas=metadatas)

        # Search with filter
        results = db.search(
            "algorithms and research", n_results=2, where={"category": "AI"}
        )
