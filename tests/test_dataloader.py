...
def test_dataset_length(sample_data):
    ...
    dataset = TokenDataset(sample_data)
    assert len(dataset) == len(sample_data), f"Expected {len(sample_data)}, got {len(dataset)}"

def test_collate_fn_padding(sample_data):
    """Ensure that all sequences in a batch are padded to exactly 50."""
    dataset = TokenDataset(sample_data)
    dataloader = DataLoader(dataset, batch_size=2, shuffle=False, collate_fn=lambda batch: collate_fn(batch, max_len=50))
