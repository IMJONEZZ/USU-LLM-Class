...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Query the fine-tuned Llama model with RAG support"
    )
    ...
    # Query configuration
    parser.add_argument(
        "--question", type=str, required=True, help="Question to ask the model"
    )
    parser.add_argument(
        "--use-rag", action="store_true", default=True, help="Use RAG during querying"
    )
    parser.add_argument(
        "--no-rag",
        action="store_false",
    ...
    parser.add_argument(
        "--compare", action="store_true", help="Compare results with and without RAG"
    )
    parser.add_argument(
...
def query_model(
    ...
    if use_rag and vectordb_info is not None:
        ...
        context_results = retrieve_similar_context(
            question=question, vectordb_info=vectordb_info, n_results=rag_results
        )
    ...
    with torch.no_grad():
        outputs = model.generate(
            inputs.input_ids, attention_mask=inputs.attention_mask, **generation_params
        )
