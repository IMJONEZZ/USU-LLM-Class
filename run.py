...
class TokenDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return torch.tensor(self.tokenized_sequences[idx], dtype=torch.long)  # No padding here
...
def collate_fn(batch, max_len=50):
    ...
    if padded_batch.shape[1] < max_len:
        ...
        padded_batch = torch.cat([padded_batch, torch.zeros((padded_batch.shape[0], pad_amount), dtype=torch.long)], dim=1)
...
# --------------------------------------------------
@step
def train_model(token_ids: List[List[int]], batch_size: int = 8, num_workers: int = 2) -> None:
